(function () {
  ...
  function setCookies(cookies, callback) {
    if (cookies !== null && typeof cookies === "object") {
      ...
      callback(
        new Error(
          "Invalid cookie format, should be an object. COOKIE_JAR not changed."
        ),
        false
      );
    }
  ...
  function executeApiCall(callback, needConnection) {
    needConnection =
      typeof needConnection !== "undefined" ? needConnection : true;
  ...
  function add(torrent, dlPath, callback) {
    if (validUrl.isWebUri(torrent)) {
      downloadTorrentFile(torrent, searchCookieJar(torrent), function (
        error,
        result
      ) {
        if (error) {
